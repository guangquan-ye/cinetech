...
const tvWidth = tvSlider.offsetWidth / 0; // Largeur d'une série (ajustez la valeur selon vos besoins)
let tvIsDown = false;
let tvStartX;
let tvScrollLeft;
...
tvSlider.addEventListener('mousemove', e => {
  ...
  const tvWalk = (tvX - tvStartX) * 1.5; // Ajustez la vitesse de défilement selon vos besoins
  const tvMaxScrollLeft = tvSlider.scrollWidth - tvSlider.offsetWidth; // Limite maximale du défilement
  const tvNewScrollLeft = tvScrollLeft - tvWalk;
  tvSlider.scrollLeft = Math.max(0, Math.min(tvMaxScrollLeft, tvNewScrollLeft)); // Empêche le défilement de sortir de la div
});
...
async function createCarousel() {
  ...
  try {
    ...
    function showTvShows(index) {
      ...
      for (let i = index; i < endIndex; i++) {
        ...
        if (tvshow) {
          tvshowDisplayDiv.innerHTML += `
            <div class="tvshowDiv">
              <img src="https://image.tmdb.org/t/p/w500/${tvshow.poster_path}" alt="${tvshow.original_title} Poster">
            </div>`;
        }
      }
    }
    ...
    console.error("Une erreur s'est produite lors de la récupération des séries :", error);
}
}
createCarousel();
